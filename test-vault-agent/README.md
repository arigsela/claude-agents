...
Test agent for vault setup verification
...
- **Test Agent** (port 8080): Answer test questions
...
- ECR repositories (`test-vault-agent-orchestrator` and `test-vault-agent-test-agent`) are created automatically
- Docker images are built and pushed to ECR during scaffolding

**Post-merge steps** (after the K8s PR is merged):

1. Replace placeholder Vault secrets with real values at path `k8s-secrets/data/test-vault-agent`
...
Edit `src/test-agent/tools.py` to add domain-specific tools.
...
1. Create a new directory under `src/` following the pattern of `src/test-agent/`
2. Add a Dockerfile in `docker/`
3. Add K8s manifests in the [arigsela/kubernetes](https://github.com/arigsela/kubernetes) repo under `base-apps/test-vault-agent/`
...
│   └── test-agent/  # Sub-agent with tools and knowledge
