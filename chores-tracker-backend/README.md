...
AI agent with deep knowledge of the Chores Tracker application
...
- **Chores Tracker Backend Knowledge specialist** (port 8080): answer questions about the chores Tracker backend  API, deployment, troubleshooting using knowledge sources
...
- ECR repositories (`chores-tracker-backend-orchestrator` and `chores-tracker-backend-knowledge-agent`) are created automatically
- Docker images are built and pushed to ECR during scaffolding

**Post-merge steps** (after the K8s PR is merged):

1. Replace placeholder Vault secrets with real values at path `k8s-secrets/data/chores-tracker-backend`
...
3. Add K8s manifests in the [arigsela/kubernetes](https://github.com/arigsela/kubernetes) repo under `base-apps/chores-tracker-backend/`
