...
Test agent for verifying oncall-crewai directory structure
...
- **Test Sub Agent** (port 8080): Verify the template creates K8s manifests in the correct oncall-crewai directory
...
GitOps repo under `base-apps/oncall-crewai/test-retest-agent/`. The Backstage
...
- ECR repositories (`test-retest-agent-orchestrator` and `test-retest-agent-test-sub-agent`) are created automatically
...
1. Replace placeholder Vault secrets with real values at path `k8s-secrets/data/test-retest-agent`
...
Edit `src/test-sub-agent/tools.py` to add domain-specific tools.
...
1. Create a new directory under `src/` following the pattern of `src/test-sub-agent/`
2. Add a Dockerfile in `docker/`
3. Add K8s manifests in the [arigsela/kubernetes](https://github.com/arigsela/kubernetes) repo under `base-apps/oncall-crewai/test-retest-agent/`
...
│   └── test-sub-agent/  # Sub-agent with tools and knowledge
