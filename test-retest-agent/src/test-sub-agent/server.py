...
#   uvicorn test-sub-agent.server:app --host 0.0.0.0 --port 8080
# ==============================================================================
...
logger = setup_logging("test-sub-agent.server")
...
AGENT_CARD = AgentCard(
    name="Test Sub Agent",
    description="Verify the template creates K8s manifests in the correct oncall-crewai directory",
...
def create_app() -> FastAPI:
    ...
    application = FastAPI(
        title=f"{PROJECT_NAME} - Test Sub Agent",
        version="1.0.0",
    )
    ...
    async def health():
        return {"status": "healthy", "service": "test-sub-agent"}
    ...
    from test-sub-agent.executor import SubAgentExecutor
