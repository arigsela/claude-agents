...
- Uses Kubernetes MCP server for cluster operations (native Go, no kubectl)
- Uses GitHub MCP server for deployment correlation
- Uses Atlassian MCP server for Jira incident tracking
...
class EKSMonitoringDaemon:
    ...
    async def initialize_client(self):
        ...
        """
        self.log("Initializing Claude Agent SDK client with MCP servers...")

        # Get project root directory
        project_root = Path(__file__).parent.absolute()

        # MCP server binary paths
        k8s_mcp_bin = project_root / "bin" / "run-kubernetes-mcp.sh"
        github_mcp_bin = project_root / "bin" / "run-github-mcp.sh"
        atlassian_mcp_bin = project_root / "bin" / "run-atlassian-mcp.sh"

        # Verify binaries exist
        if not k8s_mcp_bin.exists():
            raise FileNotFoundError(
                f"Kubernetes MCP server not found at {k8s_mcp_bin}\n"
                f"Build it with: ./scripts/build-mcp-servers.sh"
            )
        if not github_mcp_bin.exists():
            raise FileNotFoundError(
                f"GitHub MCP server not found at {github_mcp_bin}\n"
                f"Build it with: ./scripts/build-mcp-servers.sh"
            )
        if not atlassian_mcp_bin.exists():
            raise FileNotFoundError(
                f"Atlassian MCP server wrapper not found at {atlassian_mcp_bin}\n"
                f"This should have been created during setup."
            )

        # Verify GitHub token is set
        if not os.getenv('GITHUB_PERSONAL_ACCESS_TOKEN'):
            self.log("WARNING: GITHUB_PERSONAL_ACCESS_TOKEN not set - GitHub MCP will fail", level="WARN")

        # Verify Jira credentials are set (optional - agent works without Jira)
        if not os.getenv('JIRA_URL'):
            self.log("INFO: JIRA_URL not set - Jira integration disabled", level="INFO")
        elif not os.getenv('JIRA_API_TOKEN'):
            self.log("WARNING: JIRA_API_TOKEN not set - Jira MCP will fail", level="WARN")
        ...
        options = ClaudeAgentOptions(
            ...
            # SDK spawns these as child processes and communicates via JSON-RPC
            mcp_servers={
                # Kubernetes MCP Server (Go binary, no kubectl dependency)
                "kubernetes": {
                    "command": str(k8s_mcp_bin),
                    "args": [],
                    "env": {
                        "KUBECONFIG": os.getenv("KUBECONFIG", ""),
                        "KUBE_CONTEXT": KUBE_CONTEXT,  # Specify which context to use
                        "K8S_MCP_LOG_LEVEL": os.getenv("K8S_MCP_LOG_LEVEL", "2"),
                        "K8S_MCP_READ_ONLY": os.getenv("K8S_MCP_READ_ONLY", "false"),
                        "K8S_MCP_DISABLE_DESTRUCTIVE": os.getenv("K8S_MCP_DISABLE_DESTRUCTIVE", "false"),
                    }
                },

                # GitHub MCP Server (Go binary in stdio mode)
                "github": {
                    "command": str(github_mcp_bin),
                    "args": [],
                    "env": {
                        "GITHUB_PERSONAL_ACCESS_TOKEN": os.getenv("GITHUB_PERSONAL_ACCESS_TOKEN", ""),
                        "GITHUB_TOOLSETS": os.getenv("GITHUB_TOOLSETS", "context,repos,issues,pull_requests"),
                        "GITHUB_READ_ONLY": os.getenv("GITHUB_READ_ONLY", "false"),
                    }
                },

                # Atlassian MCP Server (Docker container via wrapper script)
                "atlassian": {
                    "command": str(atlassian_mcp_bin),
                    "args": [],
                    "env": {
                        ...
                        "READ_ONLY_MODE": os.getenv("JIRA_READ_ONLY", "false"),
                    }
                }
            },
            ...
            allowed_tools=[
                ...
                "Bash",        # Shell commands (for kubectl workaround - MCP K8s server is broken)

                # Kubernetes MCP tools - MOST DISABLED due to namespace filtering bug
                # See: docs/incidents/2025-10-15-MCP-KUBERNETES-SERVER-BUG.md
                # The MCP server returns ALL pods regardless of namespace filter = token limit errors

                # DISABLED (broken - return entire cluster):
                ...
                # ENABLED (still work for single-resource operations):
                "mcp__kubernetes__pods_get",      # Get single pod (works when you have exact name)
                "mcp__kubernetes__pods_log",      # Get logs for specific pod (works)
                "mcp__kubernetes__pods_exec",     # Exec into pod (works)
                "mcp__kubernetes__configuration_contexts_list",  # Config operations (work)
                "mcp__kubernetes__configuration_view",

                # Resource management tools (still enabled for remediation)
                "mcp__kubernetes__resources_get",
                "mcp__kubernetes__resources_create_or_update",
                "mcp__kubernetes__resources_delete",

                # GitHub MCP tools (from github/github-mcp-server)
                # Context toolset
                "mcp__github__get_me",
                ...
                # Atlassian MCP tools (Jira only - focused on incident tracking)
...
"""
        ...
        self.log(f"MCP servers: kubernetes (Go), github (Go){', ' + ', '.join(mcp_summary) if mcp_summary else ''}")
        ...
        while self.running:
            try:
                ...
                async with self.client:
                    ...
                    self.log("MCP servers spawned and ready")
