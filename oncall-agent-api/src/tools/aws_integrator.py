...
class AWSIntegrator:
    ...
    def __init__(self, region: str | None = None):
        ...
        try:
            ...
            logger.warning(
                "AWSIntegrator initialized but boto3 not available - operations will fail gracefully"
            )
        ...
        try:
            ...
            for ext_secret in external_secrets:
                ...
                if not aws_secret_path:
                    logger.warning(
                        f"ExternalSecret '{k8s_name}' has no AWS secret path, skipping verification"
                    )
                    continue

                logger.debug(
                    f"Checking AWS Secrets Manager for ExternalSecret '{k8s_name}' → AWS path: '{aws_secret_path}'"
                )
                ...
                try:
                    ...
                    logger.info(
                        f"  ✓ AWS secret at path '{aws_secret_path}' EXISTS in Secrets Manager"
                    )
                    ...
                    if error_code == "ResourceNotFoundException":
                        ...
                        logger.warning(
                            f"  ✗ AWS secret at path '{aws_secret_path}' NOT FOUND in Secrets Manager"
                        )
                        ...
                        logger.warning(
                            f"  ⚠ Access denied checking '{aws_secret_path}' - IAM permission issue (secret may exist)"
                        )
                        ...
                        logger.warning(
                            f"  ⚠ Error checking '{aws_secret_path}': {error_code} - {e.response.get('Error', {}).get('Message', '')}"
                        )
                    ...
                    verification_results.append(
                        {
                            ...
                            "pushed_at": (
                                image_details.get("imagePushedAt").isoformat()
    ...
    def get_ecr_image_details(self, repository: str, tag: str = "latest") -> dict[str, Any] | None:
        """
        ...
        try:
            ...
            return {
                ...
                "pushed_at": (
                    details.get("imagePushedAt").isoformat()
                    if details.get("imagePushedAt")
                    else None
                ),
    ...
    def parse_ecr_uri(self, image_uri: str) -> dict[str, str] | None:
        """
    ...
    async def get_vpc_endpoints(self, vpc_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "has_databricks_endpoint": any(
                    "databricks" in e.get("service_name", "").lower()
                    for e in endpoints_by_service.values()
                ),
            }
