...
class DatadogIntegrator:
    ...
    def __init__(
        ...
        try:
            ...
            logger.info(
                f"DatadogIntegrator initialized for site: {self.site} (datadog-api-client available)"
            )
        except ImportError:
            self.datadog_available = False
            logger.warning(
                "DatadogIntegrator initialized but datadog-api-client not available - operations will fail gracefully"
            )
    ...
    @property
    def logs_api(self):
        """Lazy initialization of Logs API client."""
        if not self.datadog_available:
            return None

        if not hasattr(self, "_logs_api") or self._logs_api is None:
            from datadog_api_client.v2.api.logs_api import LogsApi

            if self.api_client is None:
                logger.error("Cannot initialize LogsApi - API client not available")
                return None

            self._logs_api = LogsApi(self.api_client)

        return self._logs_api

    def query_timeseries(self, query: str, start: int, end: int) -> dict[str, Any]:
        """
        ...
        try:
            ...
            response = self.metrics_api.query_metrics(_from=start, to=end, query=query)

            # Convert response to dictionary
            result = {"query": query, "from_ts": start, "to_ts": end, "series": []}
    ...
    async def query_pod_metrics(
        ...
        logger.info(
            f"Querying pod metrics: {metric} for namespace={namespace}, pod={pod_name or 'all'}, hours={hours_back}"
        )

        result = self.query_timeseries(
            query=query, start=int(start_time.timestamp()), end=int(end_time.timestamp())
        )

        return result

    async def query_container_metrics(
        self, namespace: str, container_name: str | None = None, hours_back: int = 1
    ) -> dict[str, Any]:
        """
        ...
        logger.info(
            f"Querying container metrics for namespace={namespace}, container={container_name or 'all'}, hours={hours_back}"
        )
        ...
        for metric in metrics_to_query:
            ...
            metric_data = self.query_timeseries(query=query, start=start_ts, end=end_ts)
    ...
    async def query_network_metrics(
        self, namespace: str, pod_name: str | None = None, hours_back: int = 1
    ) -> dict[str, Any]:
        """
        ...
        logger.info(
            f"Querying network metrics for namespace={namespace}, pod={pod_name or 'all'}, hours={hours_back}"
        )
    ...
    def query_logs(
        self, query: str, start: datetime, end: datetime, limit: int = 1000, sort: str = "timestamp"
    ) -> dict[str, Any]:
        """
        Query Datadog logs using the Logs API.

        This method queries the Datadog Logs API for log events matching a query
        over a specified time range. Useful for analyzing Zeus orchestrator logs,
        zeus-web logs, and correlating with job execution.

        Args:
            query: Datadog log query (e.g., "service:zeus-orchestrator kube_namespace:qa")
        ...
        Example query formats:
            - "service:zeus-orchestrator kube_namespace:qa"
            - "service:zeus-web kube_namespace:devmatt @pod_name:zeus-web-*"
            - "kube_namespace:qa @refresh_type:databricks"
            - "service:zeus-orchestrator ERROR"
        ...
        Example return:
            {
                "logs": [
                    {
                        ...
                        "message": "Starting refresh for client ABC",
                        "attributes": {
                            "service": "zeus-orchestrator",
                            "kube_namespace": "qa",
                            "pod_name": "zeus-refresh-abc-12345",
                            ...
                        }
                    },
                    ...
                ...
                "total": 150,
                "query": "service:zeus-orchestrator kube_namespace:qa"
            }
        """
        if not self.datadog_available:
            logger.warning("Cannot query Datadog logs - datadog-api-client not available")
            return {
                "error": "datadog-api-client not installed",
                "message": "Install with: pip install datadog-api-client",
            }
        ...
        if not self.logs_api:
            logger.error("Logs API client not initialized")
            return {
                "error": "Datadog client not initialized",
                "message": "Check DATADOG_API_KEY and DATADOG_APP_KEY environment variables",
            }
        ...
        try:
            ...
            logger.info(f"Querying Datadog logs: {query} from {start} to {end}")

            # Build request with ISO format timestamps
            body = LogsListRequest(
                filter=LogsQueryFilter(
                    query=query,
                    _from=start.isoformat(),  # Use _from instead of from_
                    to=end.isoformat(),
                ),
                sort=LogsSort(sort),
            ...
            if response.data:
                for log in response.data:
                    log_entry = {
                        ...
                        "message": (
                            log.attributes.message if hasattr(log.attributes, "message") else None
                        ),
                        "status": (
                            log.attributes.status if hasattr(log.attributes, "status") else None
                        ),
                        "service": (
                            log.attributes.service if hasattr(log.attributes, "service") else None
                        ),
                        "attributes": {},
                    }

                    # Extract custom attributes
                    if hasattr(log.attributes, "attributes") and log.attributes.attributes:
                        log_entry["attributes"] = _to_json_serializable(log.attributes.attributes)

                    result["logs"].append(log_entry)

                result["total"] = len(result["logs"])
                logger.info(f"✓ Retrieved {result['total']} log entries from Datadog")
            else:
                logger.warning(f"No logs returned for query: {query}")
                result["message"] = "No logs available for this query and time range"

            return result

        except Exception as e:
            logger.error(f"Error querying Datadog logs: {e}")
            return {
                "error": str(e),
                "query": query,
                "message": "Datadog Logs API error - check credentials and query syntax",
            }


# Global singleton instance
_datadog_integrator = None


def get_datadog_integrator() -> DatadogIntegrator:
    """Get or create the global DatadogIntegrator instance."""
    global _datadog_integrator
    if _datadog_integrator is None:
        _datadog_integrator = DatadogIntegrator()
    return _datadog_integrator
