...
# NAT Gateway configuration - supports multiple gateways (comma-separated)
# For dev-eks: Single NAT gateway
# For prod-eks: Multiple NAT gateways (one per AZ), e.g., "nat-1a,nat-1b,nat-1c"
_NAT_GATEWAY_IDS_RAW = os.getenv("NAT_GATEWAY_IDS", os.getenv("NAT_GATEWAY_ID", "nat-07eb006676096fcd3"))
DEFAULT_NAT_GATEWAY_IDS: list[str] = [
    gw.strip() for gw in _NAT_GATEWAY_IDS_RAW.split(",") if gw.strip()
]
# For backwards compatibility - first gateway in list
DEFAULT_NAT_GATEWAY_ID = DEFAULT_NAT_GATEWAY_IDS[0] if DEFAULT_NAT_GATEWAY_IDS else ""
DEFAULT_VPC_ID = os.getenv("VPC_ID", "vpc-00a81b349b5975c2e")
...
@dataclass
class AggregatedNATMetrics:
    """Aggregated metrics across multiple NAT gateways"""
    ...
    total_bytes_in: float
    peak_bytes_per_second: float
    active_connections_total: float
    gateway_metrics: list[dict[str, Any]]  # Individual metrics per gateway
    all_spikes: list[dict[str, Any]]  # Spikes from all gateways
...
class NATGatewayAnalyzer:
    ...
    def fetch_nat_metrics(
        ...
        try:
            ...
            # Process data points
            bytes_out_data = sorted(
                bytes_out_response.get("Datapoints", []), key=lambda x: x["Timestamp"]
            )
            bytes_in_data = sorted(
                bytes_in_response.get("Datapoints", []), key=lambda x: x["Timestamp"]
            )
            peak_data = sorted(peak_response.get("Datapoints", []), key=lambda x: x["Timestamp"])
            connections_data = sorted(
                connections_response.get("Datapoints", []), key=lambda x: x["Timestamp"]
            )
            ...
            avg_connections = (
                sum(dp.get("Average", 0) for dp in connections_data) / len(connections_data)
                if connections_data
                else 0
            )

            # Combine data points by timestamp
            combined_data = self._combine_data_points(
                bytes_out_data, bytes_in_data, peak_data, connections_data
            )

            # Detect spikes
            spikes = self._detect_spikes(
                bytes_out_data, DEFAULT_SPIKE_THRESHOLD_GB, DEFAULT_BASELINE_GB
            )
            ...
            # Cache results
            self._cache[cache_key] = (datetime.now(UTC), metrics)

            logger.info(
                f"Fetched NAT metrics: {total_bytes_out / (1024**3):.2f} GB egress, {len(spikes)} spikes detected"
            )
            ...
            if error_code == "Throttling":
                ...
                raise ValueError(
                    "AWS CloudWatch API is being throttled. Please retry in a few seconds."
                )
            else:
                logger.error(
                    f"CloudWatch API error: {error_code} - {e.response['Error']['Message']}"
                )
                raise
    ...
    def get_nat_gateway_info(self, nat_gateway_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            info = {
                ...
                "state": nat["State"],
                "availability_zone": (
                    tags.get("Name", "unknown").split("-")[-1] if "Name" in tags else "unknown"
                ),
                "tags": tags,
                "public_ip": (
            ...
            logger.info(
                f"NAT gateway info: {nat_gateway_id} in VPC {info['vpc_id']}, AZ {info['availability_zone']}"
            )
    ...
    def _combine_data_points(
        self, bytes_out: list[dict], bytes_in: list[dict], peak: list[dict], connections: list[dict]
    ) -> list[dict[str, Any]]:
    ...
    def _detect_spikes(
        self, bytes_out_data: list[dict], spike_threshold_gb: float, baseline_gb: float
    ) -> list[dict[str, Any]]:
        """
        ...
        for dp in bytes_out_data:
            ...
            if gb_value > spike_threshold_gb:
                ...
                spikes.append(spike)
                logger.info(
                    f"Spike detected: {gb_value:.2f} GB at {spike['timestamp']} ({severity})"
                )
...
"""
    ...
    def fetch_all_nat_metrics(
        self,
        nat_gateway_ids: list[str] | None = None,
        time_window_hours: int = 1,
        start_time: datetime | None = None,
        end_time: datetime | None = None,
    ) -> AggregatedNATMetrics:
        """
        Fetch and aggregate traffic metrics from multiple NAT gateways.

        Args:
            nat_gateway_ids: List of NAT gateway IDs (default: from NAT_GATEWAY_IDS env var)
            time_window_hours: Hours to look back (default: 1, max: 168)
            start_time: Custom start time (overrides time_window_hours)
            end_time: Custom end time (default: now)

        Returns:
            AggregatedNATMetrics with combined data from all gateways
        """
        gateway_ids = nat_gateway_ids or DEFAULT_NAT_GATEWAY_IDS

        if not gateway_ids:
            raise ValueError("No NAT gateway IDs configured. Set NAT_GATEWAY_IDS environment variable.")

        logger.info(f"Fetching metrics for {len(gateway_ids)} NAT gateways: {gateway_ids}")

        # Calculate time range once for consistency
        if end_time is None:
            end_time = datetime.now(UTC)
        if start_time is None:
            start_time = end_time - timedelta(hours=time_window_hours)

        # Fetch metrics for each gateway
        all_metrics: list[NATMetrics] = []
        errors: list[str] = []

        for gw_id in gateway_ids:
            try:
                metrics = self.fetch_nat_metrics(
                ...
                logger.warning(f"Failed to fetch metrics for {gw_id}: {e}")
                errors.append(f"{gw_id}: {str(e)}")

        if not all_metrics:
            raise ValueError(f"Failed to fetch metrics from any NAT gateway. Errors: {errors}")

        # Aggregate metrics
        total_bytes_out = sum(m.total_bytes_out for m in all_metrics)
        total_bytes_in = sum(m.total_bytes_in for m in all_metrics)
        peak_throughput = max(m.peak_bytes_per_second for m in all_metrics)
        total_connections = sum(m.active_connections_avg for m in all_metrics)

        # Collect all spikes with gateway info
        all_spikes = []
        for m in all_metrics:
            for spike in m.spikes_detected:
                spike_with_gw = {**spike, "nat_gateway_id": m.nat_gateway_id, "availability_zone": m.availability_zone}
                all_spikes.append(spike_with_gw)

        # Sort spikes by timestamp
        all_spikes.sort(key=lambda x: x["timestamp"])

        # Build gateway metrics summary
        gateway_metrics = [
            {
                "nat_gateway_id": m.nat_gateway_id,
                "availability_zone": m.availability_zone,
                "total_egress_gb": round(m.total_bytes_out / (1024**3), 3),
                "total_ingress_gb": round(m.total_bytes_in / (1024**3), 3),
                "peak_mbps": round((m.peak_bytes_per_second * 8) / (1024**2), 2),
                "avg_connections": round(m.active_connections_avg, 0),
                "spikes_count": len(m.spikes_detected),
            }
            for m in all_metrics
        ]

        aggregated = AggregatedNATMetrics(
            nat_gateway_ids=[m.nat_gateway_id for m in all_metrics],
            vpc_id=all_metrics[0].vpc_id,
            start_time=start_time.isoformat(),
            end_time=end_time.isoformat(),
            total_bytes_out=total_bytes_out,
            total_bytes_in=total_bytes_in,
        ...
        logger.info(
            f"Aggregated NAT metrics: {total_bytes_out / (1024**3):.2f} GB total egress, "
            f"{len(all_spikes)} spikes across {len(all_metrics)} gateways"
        )

        return aggregated

    def format_aggregated_metrics_for_llm(self, metrics: AggregatedNATMetrics) -> str:
        """
        Format aggregated metrics as human-readable string for LLM consumption.

        Args:
            metrics: AggregatedNATMetrics object

        Returns:
            Formatted string with combined metrics and per-gateway breakdown
        """
        total_gb_out = metrics.total_bytes_out / (1024**3)
        total_gb_in = metrics.total_bytes_in / (1024**3)
        peak_mbps = (metrics.peak_bytes_per_second * 8) / (1024**2)

        output = f"""NAT Gateway Traffic Analysis (Multi-Gateway)
Gateways: {len(metrics.nat_gateway_ids)} ({', '.join(metrics.nat_gateway_ids)})
VPC: {metrics.vpc_id}
Time Period: {metrics.start_time} to {metrics.end_time}
...
- Total Ingress: {total_gb_in:.3f} GB
- Peak Throughput: {peak_mbps:.2f} Mbps
- Total Active Connections: {metrics.active_connections_total:.0f}

Per-Gateway Breakdown:
"""
        for gw in metrics.gateway_metrics:
            output += f"  • {gw['nat_gateway_id']} ({gw['availability_zone']}): "
            output += f"{gw['total_egress_gb']} GB egress, {gw['peak_mbps']} Mbps peak"
            if gw['spikes_count'] > 0:
                output += f", {gw['spikes_count']} spikes"
            output += "\n"

        output += "\n"

        if metrics.all_spikes:
            output += f"⚠️ Traffic Spikes Detected: {len(metrics.all_spikes)} total\n\n"
            for spike in metrics.all_spikes:
                output += f"  • {spike['timestamp']} ({spike['nat_gateway_id']}): {spike['bytes_transferred_gb']} GB "
                output += f"({spike['multiplier']}x baseline, severity: {spike['severity']})\n"
        else:
            output += "✅ No significant traffic spikes detected across any gateway\n"

        return output


# Global instance (initialized lazily)
_analyzer: NATGatewayAnalyzer | None = None
