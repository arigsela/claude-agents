...
class TeamsNotifier:
    ...
    def __init__(self, webhook_url: str, config: dict | None = None, dry_run: bool = False):
        """
    ...
    async def send_incident_alert(
        self, incident: dict, severity: str, diagnosis: dict | None = None
    ) -> dict[str, Any]:
        """
        ...
        if not self._should_notify(severity, service):
            logger.info(
                f"Skipping notification for {service} ({severity}) - rate limited or below threshold"
            )
    ...
    async def send_diagnosis_complete(
        self, incident: dict, diagnosis: dict, recommendations: dict
    ) -> dict[str, Any]:
        """
    ...
    async def send_escalation_alert(
        self, incident: dict, reason: str, diagnosis: dict | None = None
    ) -> dict[str, Any]:
        """
    ...
    def _should_notify(self, severity: str, service: str) -> bool:
        ...
        if last_notification:
            ...
            if time_since < timedelta(minutes=cooldown_minutes):
                logger.info(
                    f"Notification rate limited: {service} (cooldown: {cooldown_minutes}min)"
                )
                return False
    ...
    def _build_incident_card(self, incident: dict, severity: str, diagnosis: dict | None) -> dict:
        """
            ...
            "attachments": [
                {
                    ...
                    "content": {
                        ...
                        "actions": [
                            {"type": "Action.OpenUrl", "title": "View in ArgoCD", "url": argocd_url}
                        ],
                    },
                }
            ],
    ...
    def _build_diagnosis_card(self, incident: dict, diagnosis: dict, recommendations: dict) -> dict:
        """
        ...
        root_cause = llm_analysis.get("root_cause_refined") or llm_analysis.get(
            "root_cause", "Unknown"
        )
        ...
        if not steps:
            steps = diagnosis.get("remediation_guidance", {}).get(
                "recommended_actions",
                ["Check pod logs", "Review deployment configuration", "Investigate root cause"],
            )

        # Kubectl commands (can be extracted from steps or kept generic)
        [step for step in steps if "kubectl" in step.lower()][:3]
                    ...
                    "content": {
                        ...
                        "body": [
                            ...
                            },
                            {
                                "type": "TextBlock",
                                "text": (
                                    "\n".join(f"{i}. {step}" for i, step in enumerate(steps[:8], 1))
                                    if steps
                                    else "No specific steps available"
                                ),
                                "wrap": True,
                            },
                        ],
    ...
    def _build_escalation_card(self, incident: dict, reason: str, diagnosis: dict | None) -> dict:
        """
            ...
            "attachments": [
                {
                    ...
                    "content": {
                        ...
                        "body": [
                            ...
                            {"type": "TextBlock", "text": "**What to Do:**", "weight": "Bolder"},
                            {
                                "type": "TextBlock",
    ...
    def get_notification_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "services_notified": list({key.split("_")[0] for key in self.notification_cache}),
            "oldest_notification": (
                min(self.notification_cache.values()) if self.notification_cache else None
            ),
            "newest_notification": (
                max(self.notification_cache.values()) if self.notification_cache else None
            ),
        }
