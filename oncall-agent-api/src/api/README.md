...
- **Cluster Protection**: Controlled via `ALLOWED_CLUSTERS` env var (comma-separated); unauthorized clusters rejected with 422 error
...
FastAPI Server (lifespan managed)
    ↓
OnCallAgentClient (initialized at startup)
    ↓
Direct Anthropic API + Python K8s/GitHub/AWS libraries
    ↓
JSON Response
```
