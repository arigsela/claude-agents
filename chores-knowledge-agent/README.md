# chores-knowledge-agent

AI agent with deep knowledge of the Chores Tracker application architecture, API, deployment, and troubleshooting
...
- ECR repositories (`chores-knowledge-agent-orchestrator` and `chores-knowledge-agent-knowledge-agent`) are created automatically
- Docker images are built and pushed to ECR during scaffolding

**Post-merge steps** (after the K8s PR is merged):

1. Add the `chores-knowledge-agent` namespace to the ECR auth CronJob in `base-apps/ecr-auth/cronjobs.yaml`
2. Create Vault role and secrets at path `k8s-secrets/data/chores-knowledge-agent`
3. ArgoCD will auto-deploy once manifests are in the `main` branch
...
Then update `src/knowledge-agent/tools.py` to read from these files.
...
3. Add K8s manifests in the [arigsela/kubernetes](https://github.com/arigsela/kubernetes) repo under `base-apps/chores-knowledge-agent/`
